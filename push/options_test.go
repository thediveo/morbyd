...
var _ = Describe("push image options", func() {

	It("processes push image options", func() {
		popts := Options{}
		for _, opt := range []Opt{
			WithAllTags(),
		...
		Expect(popts.Platform).To(And(
			HaveField("OS", "sunkenos"),
			HaveField("Architecture", "sparq")))
